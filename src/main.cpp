...
uint256 hashGenesisBlock("0x34ac0e233e264282aadda2f9b884090e255b77d2cfff9dea50e7648075e5ff5d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zeuscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zeuscoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	int rand = generateMTRandom(seed, 901);

	nSubsidy = (100 + rand - 1) * COIN;

	// bonus days
	if((nHeight < 8641) || (nHeight > 48960 && nHeight < 57601))	
	{
		nSubsidy *= 3;
	}
	else if(nHeight > 17280 && nHeight < 25921)	
	{
		nSubsidy *= 2;
	}
	else if((nHeight > 85320 && nHeight < 86401) || (nHeight > 169920 && nHeight < 172801))	
	{
		nSubsidy *= 5;
	}
...
static const int64 nTargetTimespan = 30 * 30;	// Zeuscoin: 15 mins 
static const int64 nTargetSpacing = 30;			// Zeuscoin: 30 sec
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal zeuscoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Zeuscoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x16d116fdde8c059da620a7e029181ce748beb9c39a1939edcad0b6ac51375f53");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "October 6, 2013: Bitcointalk is back online after down 4 days following a hack of the site, which happened hours after Silk Road takedown";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0441234678901234567890077771222333444555666777888999000000aaaaabbbbbcccccdddddeeeeeff00ff00ff00ff001234567890abcdef0022446688abc89") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1380958086;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 13577531;

		if (fTestNet)
        {
            block.nTime    = 1380000000;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb87039d2289284a01c0ce5fbd4c931a15045114cdb44d05a1cf7ff54a81500f4"));
...
unsigned char pchMessageStart[4] = { 0xb6, 0xc0, 0xa5, 0xc1 }; 
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Zeuscoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
